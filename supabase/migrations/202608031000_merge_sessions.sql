...
begin
  ...
  if v_target.owner_id <> p_owner_id
     or v_source.owner_id <> p_owner_id
     or v_target.owner_id <> v_source.owner_id then
...
revoke all on function merge_sessions(text, text, text, bigint, bigint, jsonb) from public;
grant execute on function merge_sessions(text, text, text, bigint, bigint, jsonb) to service_role;
