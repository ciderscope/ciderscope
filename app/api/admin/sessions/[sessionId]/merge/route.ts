...
const loadMergeSessions = async (
  ...
  sourceSessionId: string
): Promise<{ target: SessionRow; source: SessionRow } | NextResponse> => {
  ...
  if (
    !target
    || !source
    || !canAccessOwner(auth.user, target.owner_id)
    || !canAccessOwner(auth.user, source.owner_id)
  ) {
    return NextResponse.json({ error: "Séance introuvable." }, { status: 404 });
  }
  if (target.owner_id !== source.owner_id) {
    return NextResponse.json({ error: "Les deux séances doivent appartenir à la même entité." }, { status: 409 });
  }
...
export async function GET(
  ...
  try {
    ...
    const sessions = await loadMergeSessions(request, sessionId, sourceSessionId);
...
export async function POST(
  ...
  try {
    ...
    const result = await executeSessionMerge({
      ...
      sourceSessionId,
      ownerId: sessions.target.owner_id,
    ...
    if (!result.ok) {
      const messages: Record<string, string> = {
        ...
        owner_mismatch: "Les deux séances doivent appartenir à la même entité.",
